...
namespace Web.YFC.Services
{
	public class CommunityAlbumServices
	{
		...
		public async Task<List<CommunityAlbum>> GetCommunityAlbumByComunityId(int id)
		{
			List<CommunityAlbum> CommunityAlbum = new List<CommunityAlbum>();

			var result = await RestCall.Get(AppSettings.ApiUri + EndPoints.CommunityAlbumEndpoint + "/ByCommunityId/" + id);
			if (!string.IsNullOrWhiteSpace(result))
			{
				CommunityAlbum = JsonSerializer.Deserialize<List<CommunityAlbum>>(result, AppSettings.options)!;
			}
			return CommunityAlbum;
		}
