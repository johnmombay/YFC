...
namespace Web.YFC.Services
{
	public class CommunityInfoServices
	{
		...
		public async Task<CommunityInfo> GetCommunityInfosByCommunityId(int id)
		{
			CommunityInfo CommunityInfo = new CommunityInfo();

			var result = await RestCall.Get(AppSettings.ApiUri + EndPoints.CommunityInfoEndpoint + "/GetCommunityInfosByCommunityId/" + id);
			if (!string.IsNullOrWhiteSpace(result))
			{
				CommunityInfo = JsonSerializer.Deserialize<CommunityInfo>(result, AppSettings.options)!;
			}
