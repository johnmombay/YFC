...
namespace Web.YFC.Services
{
	public class CommunityArticleServices
	{
		...
		public async Task<List<CommunityArticle>> GetCommunityArticlesByCommunityId(int id)
		{
			List<CommunityArticle> contacts = new List<CommunityArticle>();

			var result = await RestCall.Get(AppSettings.ApiUri + EndPoints.CommunityArticleEndpoint + "/GetCommunityArticlesByCommunityId/" + id);
			if (!string.IsNullOrWhiteSpace(result))
			{
				contacts = JsonSerializer.Deserialize<List<CommunityArticle>>(result, AppSettings.options)!;
			}
