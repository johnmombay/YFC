...
namespace Web.YFC.Services
{
	public class MinistryLeaderServices
	{
		...
		public async Task<List<MinistryLeader>> GetMinistryLeadersByMinistryId(int id)
		{
			List<MinistryLeader> contacts = new List<MinistryLeader>();

			var result = await RestCall.Get(AppSettings.ApiUri + EndPoints.MinistryLeaderEndpoint + "/GetMinistryLeadersByMinistryId/" + id);
			if (!string.IsNullOrWhiteSpace(result))
			{
				contacts = JsonSerializer.Deserialize<List<MinistryLeader>>(result, AppSettings.options)!;
			}
