...
namespace Web.YFC.Services
{
	public class CommunityLeaderServices
	{
		...
		public async Task<List<CommunityLeader>> GetCommunityLeadersByCommunityId(int id)
		{
			List<CommunityLeader> contacts = new List<CommunityLeader>();

			var result = await RestCall.Get(AppSettings.ApiUri + EndPoints.CommunityLeaderEndpoint + "/GetCommunityLeadersByCommunityId/" + id);
			if (!string.IsNullOrWhiteSpace(result))
			{
				contacts = JsonSerializer.Deserialize<List<CommunityLeader>>(result, AppSettings.options)!;
			}
