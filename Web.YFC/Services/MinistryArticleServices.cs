...
namespace Web.YFC.Services
{
	public class MinistryArticleServices
	{
		...
		public async Task<List<MinistryArticle>> GetMinistryArticlesByMinistryId(int id)
		{
			List<MinistryArticle> contacts = new List<MinistryArticle>();

			var result = await RestCall.Get(AppSettings.ApiUri + EndPoints.MinistryArticleEndpoint + "/GetMinistryArticlesByMinistryId/" + id);
			if (!string.IsNullOrWhiteSpace(result))
			{
				contacts = JsonSerializer.Deserialize<List<MinistryArticle>>(result, AppSettings.options)!;
			}
