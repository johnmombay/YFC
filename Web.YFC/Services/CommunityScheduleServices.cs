...
namespace Web.YFC.Services
{
	public class CommunityScheduleServices
	{
		...
		public async Task<List<CommunitySchedule>> GetCommunitySchedulesByCommunityId(int id)
		{
			List<CommunitySchedule> contacts = new List<CommunitySchedule>();

			var result = await RestCall.Get(AppSettings.ApiUri + EndPoints.CommunityScheduleEndpoint + "/GetCommunitySchedulesByCommunityId/" + id);
			if (!string.IsNullOrWhiteSpace(result))
			{
				contacts = JsonSerializer.Deserialize<List<CommunitySchedule>>(result, AppSettings.options)!;
			}
