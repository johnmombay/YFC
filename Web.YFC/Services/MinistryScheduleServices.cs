...
namespace Web.YFC.Services
{
	public class MinistryScheduleServices
	{
		...
		public async Task<List<MinistrySchedule>> GetMinistrySchedulesByMinistryId(int id)
		{
			List<MinistrySchedule> contacts = new List<MinistrySchedule>();

			var result = await RestCall.Get(AppSettings.ApiUri + EndPoints.MinistryScheduleEndpoint + "/GetMinistrySchedulesByMinistryId/" + id);
			if (!string.IsNullOrWhiteSpace(result))
			{
				contacts = JsonSerializer.Deserialize<List<MinistrySchedule>>(result, AppSettings.options)!;
			}
