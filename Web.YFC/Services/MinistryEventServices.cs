...
namespace Web.YFC.Services
{
	public class MinistryEventServices
	{
		...
		public async Task<List<MinistryEvent>> GetMinistryEventsByMinistryId(int id)
		{
			List<MinistryEvent> contacts = new List<MinistryEvent>();

			var result = await RestCall.Get(AppSettings.ApiUri + EndPoints.MinistryEventEndpoint + "/GetMinistryEventsByMinistryId/" + id);
			if (!string.IsNullOrWhiteSpace(result))
			{
				contacts = JsonSerializer.Deserialize<List<MinistryEvent>>(result, AppSettings.options)!;
			}
